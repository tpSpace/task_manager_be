...
import { 
  findUserByEmailMongoose, 
  createUserMongoose, 
  findUserbyUserIdMongoose 
} from '../services/userServiceMongoose';
...
export const loginUserHandlerMongoose = async (req: Request, res: Response) => {
  try {
    const { email, password } = req.body;

    const user = await findUserByEmailMongoose(email);
...
export const registerUserHandlerMongoose = async (req: Request, res: Response) => {
  try {
    ...
    const newUser = await findUserByEmailMongoose(user.email);
    const token = generateJwtToken(newUser!.userId);
...
export const getSingleUserHandlerMongoose = async (req: Request, res: Response) => {
  try {
    const id = req.params.userId;
    const user = await findUserbyUserIdMongoose(id);
    return res.status(200).json({
      status: 'success',
      user,
    });
